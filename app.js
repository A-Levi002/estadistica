...
// ════════════════════════════════════════════
// VIDEOS EDUCATIVOS (YouTube) - CORREGIDO
// ════════════════════════════════════════════
const VIDEO_LIBRARY = {
  // Estadística Descriptiva
  'media': 'https://www.youtube.com/embed/uyA_RotXQJo',
  'mediana': 'https://www.youtube.com/embed/VRJqH2lIuAA',
  'desviacion': 'https://www.youtube.com/embed/4R9z6AfV2nQ',
  'varianza': 'https://www.youtube.com/embed/3rPm9w1FJQY',
  
  // Probabilidad
  'laplace': 'https://www.youtube.com/embed/XpRRe1P_cAY',
  'bayes': 'https://www.youtube.com/embed/HZGCoVF3YvM',
  'condicional': 'https://www.youtube.com/embed/Mm5amR01lbo',
  'complemento': 'https://www.youtube.com/embed/ly3A9jj2B_Q',
  'union_excl': 'https://www.youtube.com/embed/XpRRe1P_cAY',
  'union_noexcl': 'https://www.youtube.com/embed/1-XafvRIc0s',
  'conjunta_indep': 'https://www.youtube.com/embed/HZGCoVF3YvM',
  'conjunta_dep': 'https://www.youtube.com/embed/1-XafvRIc0s',
  'diferencia': 'https://www.youtube.com/embed/XpRRe1P_cAY',
  'prob_total': 'https://www.youtube.com/embed/lLbT3v2_u9A',
  
  // Intervalos
  'ic_prop': 'https://www.youtube.com/embed/LP_WWjnNa7A',
  'ic_media': 'https://www.youtube.com/embed/HU5c6fzG8Nk',
  
  // Hipótesis
  'test_z': 'https://www.youtube.com/embed/5ABpqVSx33I',
};

// Diccionario de títulos para los videos
const VIDEO_TITLES = {
  'media': 'Media aritmética (promedio)',
  'mediana': 'Mediana y moda',
  'desviacion': 'Desviación estándar',
  'varianza': 'Varianza muestral',
  'laplace': 'Regla de Laplace',
  'bayes': 'Teorema de Bayes',
  'condicional': 'Probabilidad condicional',
  'complemento': 'Evento complementario',
  'union_excl': 'Unión de eventos excluyentes',
  'union_noexcl': 'Unión de eventos no excluyentes',
  'conjunta_indep': 'Probabilidad conjunta (independientes)',
  'conjunta_dep': 'Probabilidad conjunta (dependientes)',
  'diferencia': 'Diferencia de eventos',
  'prob_total': 'Probabilidad total',
  'ic_prop': 'Intervalo de confianza para proporción',
  'ic_media': 'Intervalo de confianza para media',
  'test_z': 'Prueba Z para proporción'
};

function getVideoUrl(topic) {
  // Si no existe en el diccionario, usar un video genérico de estadística
  if (!VIDEO_LIBRARY[topic]) {
    return 'https://www.youtube.com/embed/uyA_RotXQJo';
  }
  return VIDEO_LIBRARY[topic];
}

function getVideoTitle(topic) {
  return VIDEO_TITLES[topic] || 'Video explicativo';
}
...
function openVideoModal(url, title) {
  ...
  titleEl.textContent = `📺 ${title || 'Video explicativo'}`;
  iframe.src = url;
...
function normSiNo(v) {
  ...
  if (/^s/i.test(s) && s.length <= 3) return 'Sí';
...
function freqTable(horas) {
  ...
  const n = horas.length,
    relFreqs = freqs.map(f => f / n);
  let cum = 0;
  const cumFreqs = freqs.map(f => { cum += f; return cum; });
...
function updateFreqTable() {
  ...
  // ── ACTIVIDAD FÍSICA (CORREGIDO) ──
  const actividadCats = freqDicot(data.map(d => d.actividad), 'freq-actividad-body', 'spss-actividad-note');
  ...
  document.getElementById('freq-genero-body').innerHTML = genCats.map(c => {
    const fi = genVals.filter(v => v === c).length;
    const fri = fi / genVals.length;
    cumG += fi;
  ...
  document.getElementById('freq-semestre-body').innerHTML = semCats.map(c => {
    const fi = semVals.filter(v => v === c).length;
    const fri = fi / semVals.length;
    cumS += fi;
  ...
  window._chartData = { horasUniq, s, bienCats, pantCats, siestaCats, genCats, genVals, semCats, semVals,
    carCats, horCats, devCats, somCats, cafCats, actividadCats };
}
...
function calcMode(arr) {
  ...
  let mx = 0,
    mo = arr[0];
  Object.entries(freq).forEach(([k, v]) => { if (v > mx) { mx = v;
      mo = +k || k; } });
  return mo;
}
...
function updateMasterTable() {
  ...
  if (!tbody || !data.length) {
    tbody.innerHTML = '<tr><td colspan="13" style="text-align:center;color:var(--muted);padding:20px;">No hay datos cargados</td></tr>';
  ...
  tbody.innerHTML = data.map(r => `
    <tr>
      ...
      <td>${r.edad}</td>
      <td>${r.genero}</td>
      <td style="max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 10px;">${r.carrera}</td>
      <td>${r.semestre}°</td>
      <td style="font-size: 10px;">${r.horario}</td>
      <td style="color: var(--blue); font-weight: 700;">${r.horas}h</td>
      <td style="color: ${r.bien.includes('Sí') ? 'var(--green)' : 'var(--red)'};">${r.bien}</td>
      <td style="font-size: 10px; color: var(--muted2);">${r.desvela}</td>
      <td style="font-size: 10px; color: var(--muted2);">${r.somnolencia}</td>
      <td style="font-size: 10px;">${r.cafe}</td>
      <td style="color: ${r.pantalla === 'Sí' ? 'var(--purple)' : 'var(--muted2)'};">${r.pantalla}</td>
...
function exportMasterTable() {
  ...
  const headers = ['ID', 'Edad', 'Sexo', 'Carrera', 'Semestre', 'Horario', 'Horas Sueño', '¿Suficiente?', 'Desvelo', 'Somnolencia', 'Café', 'Pantallas', 'Act. Física'];
  const rows = data.map(r => [
    r.id, r.edad, r.genero, r.carrera, r.semestre, r.horario,
    r.horas, r.bien, r.desvela, r.somnolencia, r.cafe, r.pantalla, r.actividad
  ]);
  ...
  link.download = `datos_encuesta_${new Date().toISOString().slice(0, 10)}.csv`;
  link.click();
...
function updateAllCharts() {
  ...
  const base = () => ({
    ...
    scales: {
      x: { grid: { color: CO.grid }, ticks: { color: CO.ticks } },
      y: { grid: { color: CO.grid }, ticks: { color: CO.ticks }, beginAtZero: true }
    }
  });
  const pie_opts = (pos = 'bottom') => ({
    responsive: true, maintainAspectRatio: false,
    plugins: {
      legend: { position: pos, labels: { color: '#888', boxWidth: 10, font: { size: 11 } } },
      tooltip: { backgroundColor: '#111', borderColor: '#333', borderWidth: 1, titleColor: '#eee', bodyColor: '#999' }
    }
  });
  ...
  if (c1) charts.hist = new Chart(c1, {
    type: 'bar',
    data: { labels: horasUniq.map(v => v + 'h'), datasets: [{ label: 'Frecuencia', data: horasFi, backgroundColor: 'rgba(96,165,250,.3)', borderColor: 'rgba(96,165,250,.8)', borderWidth: 1.5, borderRadius: 4 }] },
    options: { ...base(), plugins: { ...base().plugins, title: { display: true, text: 'Distribución de horas de sueño', color: '#888', font: { size: 11 } } } }
  });

  // Ojiva
  let cumO = 0;
  const cumArr = [0, ...horasUniq.map(v => { cumO += s.horas.filter(h => h === v).length; return cumO; })];
  const c2 = document.getElementById('chart-ojiva');
  if (c2) charts.ojiva = new Chart(c2, {
    type: 'line',
    data: { labels: ['', ...horasUniq.map(v => v + 'h')], datasets: [{ label: 'Fi acum.', data: cumArr, borderColor: 'rgba(251,191,36,.9)', backgroundColor: 'rgba(251,191,36,.08)', tension: .35, fill: true, pointRadius: 4, pointBackgroundColor: 'rgba(251,191,36,.9)' }] },
    options: { ...base(), plugins: { ...base().plugins, title: { display: true, text: 'Ojiva — frecuencia acumulada', color: '#888', font: { size: 11 } } } }
  });

  // Duerme bien — barras
  if (cd.bienCats) {
    const bl = cd.bienCats.map(c => c.cat),
      bf = cd.bienCats.map(c => c.fi);
    ...
    if (c3) charts.bien = new Chart(c3, {
      type: 'bar',
      data: { labels: bl, datasets: [{ data: bf, backgroundColor: bColors, borderColor: bBorders, borderWidth: 1.5, borderRadius: 4 }] },
      options: { ...base(), plugins: { ...base().plugins, title: { display: true, text: '¿Duerme bien?', color: '#888', font: { size: 11 } } } }
    });
    const c3p = document.getElementById('chart-bien-pie');
    if (c3p) charts['bien-pie'] = new Chart(c3p, {
      type: 'doughnut',
      data: { labels: bl, datasets: [{ data: bf, backgroundColor: bColors, borderColor: bBorders, borderWidth: 1.5 }] },
      options: pie_opts()
    });
  }
  ...
  // Pantallas
  if (cd.pantCats) {
    const pl = cd.pantCats.map(c => c.cat),
      pf = cd.pantCats.map(c => c.fi);
    const pc = pl.map((_, i) => PALETTE[i] || PALETTE[0]),
      pcb = pl.map((_, i) => PAL_B[i] || PAL_B[0]);
    const c4b = document.getElementById('chart-pantalla-bar');
    if (c4b) charts['pantalla-bar'] = new Chart(c4b, {
      type: 'bar',
      data: { labels: pl, datasets: [{ data: pf, backgroundColor: pcb, borderColor: pc, borderWidth: 1.5, borderRadius: 4 }] },
      options: { ...base(), plugins: { ...base().plugins, title: { display: true, text: 'Uso de pantallas', color: '#888', font: { size: 11 } } } }
    });
    const c4 = document.getElementById('chart-pantalla');
    if (c4) charts.pantalla = new Chart(c4, {
      type: 'doughnut',
      data: { labels: pl, datasets: [{ data: pf, backgroundColor: pcb, borderColor: pc, borderWidth: 1.5 }] },
      options: pie_opts()
    });
  }
  ...
  // Siestas
  if (cd.siestaCats) {
    const sl = cd.siestaCats.map(c => c.cat),
      sf = cd.siestaCats.map(c => c.fi);
    const sc = sl.map((_, i) => PALETTE[i + 2] || PALETTE[0]),
      scb = sl.map((_, i) => PAL_B[i + 2] || PAL_B[0]);
    const c5b = document.getElementById('chart-siesta-bar');
    if (c5b) charts['siesta-bar'] = new Chart(c5b, {
      type: 'bar',
      data: { labels: sl, datasets: [{ data: sf, backgroundColor: scb, borderColor: sc, borderWidth: 1.5, borderRadius: 4 }] },
      options: { ...base(), plugins: { ...base().plugins, title: { display: true, text: 'Toma de siestas', color: '#888', font: { size: 11 } } } }
    });
    const c5p = document.getElementById('chart-siesta-pie');
    if (c5p) charts['siesta-pie'] = new Chart(c5p, {
      type: 'doughnut',
      data: { labels: sl, datasets: [{ data: sf, backgroundColor: scb, borderColor: sc, borderWidth: 1.5 }] },
      options: pie_opts()
    });
  }
  ...
  // Actividad Física (CORREGIDO)
  if (cd.actividadCats) {
    const al = cd.actividadCats.map(c => c.cat);
    const af = cd.actividadCats.map(c => c.fi);
    const ac = al.map((_, i) => PALETTE[i % PALETTE.length]);
    const acb = al.map((_, i) => PAL_B[i % PAL_B.length]);

    const cActBar = document.getElementById('chart-actividad-bar');
    if (cActBar) {
      destroyChart('actividad-bar');
      charts['actividad-bar'] = new Chart(cActBar, {
        type: 'bar',
        data: { labels: al, datasets: [{ data: af, backgroundColor: acb, borderColor: ac, borderWidth: 1.5, borderRadius: 4 }] },
        options: { ...base(), plugins: { ...base().plugins, title: { display: true, text: 'Actividad física regular', color: '#888', font: { size: 11 } } } }
      });
    }

    const cActPie = document.getElementById('chart-actividad-pie');
    if (cActPie) {
      destroyChart('actividad-pie');
      charts['actividad-pie'] = new Chart(cActPie, {
        type: 'doughnut',
        data: { labels: al, datasets: [{ data: af, backgroundColor: acb, borderColor: ac, borderWidth: 1.5 }] },
        options: pie_opts()
      });
    }
  }
  ...
  if (cd.genCats && cd.genVals) {
    const gl = cd.genCats,
      gf = gl.map(c => cd.genVals.filter(v => v === c).length);
    const gc = gl.map((_, i) => PALETTE[i] || PALETTE[0]),
      gcb = gl.map((_, i) => PAL_B[i] || PAL_B[0]);
    const c6b = document.getElementById('chart-genero-bar');
    if (c6b) charts['genero-bar'] = new Chart(c6b, {
      type: 'bar',
      data: { labels: gl, datasets: [{ data: gf, backgroundColor: gcb, borderColor: gc, borderWidth: 1.5, borderRadius: 4 }] },
      options: { ...base(), plugins: { ...base().plugins, title: { display: true, text: 'Distribución por género', color: '#888', font: { size: 11 } } } }
    });
    const c6p = document.getElementById('chart-genero-pie');
    if (c6p) charts['genero-pie'] = new Chart(c6p, {
      type: 'doughnut',
      data: { labels: gl, datasets: [{ data: gf, backgroundColor: gcb, borderColor: gc, borderWidth: 1.5 }] },
      options: pie_opts()
    });
  }
  ...
  if (cd.semCats && cd.semVals) {
    const sl2 = cd.semCats.map(c => c + '°'),
      sf2 = cd.semCats.map(c => cd.semVals.filter(v => v === c).length);
    const c7b = document.getElementById('chart-semestre-bar');
    if (c7b) charts['semestre-bar'] = new Chart(c7b, {
      type: 'bar',
      data: { labels: sl2, datasets: [{ data: sf2, backgroundColor: 'rgba(167,139,250,.3)', borderColor: 'rgba(167,139,250,.8)', borderWidth: 1.5, borderRadius: 4 }] },
      options: { ...base(), plugins: { ...base().plugins, title: { display: true, text: 'Frecuencia por semestre', color: '#888', font: { size: 11 } } } }
    });
    const c7l = document.getElementById('chart-semestre-line');
    if (c7l) charts['semestre-line'] = new Chart(c7l, {
      type: 'line',
      data: { labels: sl2, datasets: [{ data: sf2, borderColor: 'rgba(167,139,250,.9)', backgroundColor: 'rgba(167,139,250,.08)', tension: .3, fill: true, pointRadius: 4, pointBackgroundColor: 'rgba(167,139,250,.9)' }] },
      options: { ...base(), plugins: { ...base().plugins, title: { display: true, text: 'Polígono de frecuencias — semestre', color: '#888', font: { size: 11 } } } }
    });
  }
  ...
  function makePair(cats, barId, pieId, color, title) {
    ...
    if (cb) charts[barId] = new Chart(cb, {
      type: 'bar',
      data: { labels, datasets: [{ data: freqs, backgroundColor: colorsB, borderColor: colors, borderWidth: 1.5, borderRadius: 4 }] },
      options: { ...base(), plugins: { ...base().plugins, title: { display: true, text: title, color: '#888', font: { size: 11 } } } }
    });
    const cp = document.getElementById(pieId);
    if (cp) charts[pieId] = new Chart(cp, {
      type: 'doughnut',
      data: { labels, datasets: [{ data: freqs, backgroundColor: colorsB, borderColor: colors, borderWidth: 1.5 }] },
      options: pie_opts()
    });
  }
...
// ════════════════════════════════════════════
// PROBABILIDADES - CORREGIDO
// ════════════════════════════════════════════

// Diccionario de etiquetas para mostrar en la calculadora
const PROB_LABELS = {
  bien_si: 'Horas suficientes = Sí',
  bien_no: 'Horas suficientes = No',
  pantalla_si: 'Usa pantallas = Sí',
  pantalla_no: 'Usa pantallas = No',
  actividad_si: 'Actividad física = Sí',
  desvela_si: 'Se desvela (alguna vez)',
  horas_bajo: 'Horas sueño ≤ 6',
  horas_alto: 'Horas sueño ≥ 8'
};
...
function getLabel(k) {
  return PROB_LABELS[k] || k;
}

function calcProbInteractive() {
  const s = stats();
  if (!s.n) {
    document.getElementById('prob-result-box').innerHTML = 
      '<div class="fbox" style="color:var(--muted);text-align:center;padding:20px;">Carga datos primero en la pestaña Datos</div>';
    return;
  }
  
  const kA = document.getElementById('prob-varA').value;
  const kB = document.getElementById('prob-varB').value;
  
  // Si son la misma variable, mostrar advertencia
  if (kA === kB) {
    document.getElementById('prob-result-box').innerHTML = 
      '<div class="fbox" style="color:var(--yellow);text-align:center;padding:20px;">⚠️ Selecciona dos variables diferentes</div>';
    return;
  }
  
  const fA = getFilter(kA);
  const fB = getFilter(kB);
  const cA = data.filter(fA).length;
  const cB = data.filter(fB).length;
  const cAB = data.filter(d => fA(d) && fB(d)).length;
  
  const n = s.n;
  const pA = cA / n;
  const pB = cB / n;
  const pAB = cAB / n;
  const pAgB = cB > 0 ? cAB / cB : 0;
  const pBgA = cA > 0 ? cAB / cA : 0;
  
  const lA = getLabel(kA);
  const lB = getLabel(kB);
  
  // Verificar independencia
  const pAxB = pA * pB;
  const diff = Math.abs(pAxB - pAB);
  const areIndependent = diff < 0.03;
  
  document.getElementById('prob-result-box').innerHTML = `
    <div class="fbox">
      <span class="cm">// n = ${n} encuestados</span>
      
      <span class="cm">// Probabilidades individuales</span>
      P(A) = P(${lA}) = ${cA}/${n} = <span class="vl">${pA.toFixed(4)}</span>  [${(pA * 100).toFixed(1)}%]
      P(B) = P(${lB}) = ${cB}/${n} = <span class="vl">${pB.toFixed(4)}</span>  [${(pB * 100).toFixed(1)}%]

      <span class="cm">// Conjunta — ¿cuántos cumplen AMBAS?</span>
      P(A∩B) = ${cAB}/${n} = <span class="rs">${pAB.toFixed(4)}</span>  [${(pAB * 100).toFixed(1)}%]

      <span class="cm">// ¿Independientes? P(A)×P(B) = ${pAxB.toFixed(4)}  vs  P(A∩B) = ${pAB.toFixed(4)}</span>
      <span class="op">${areIndependent ? '≈ Aproximadamente independientes' : '≠ Dependientes (uno influye en el otro)'}</span>

      <span class="cm">// Condicional — dado que B ocurrió, ¿qué tan probable es A?</span>
      P(A|B) = P(A∩B)/P(B) = ${pAB.toFixed(4)}/${pB.toFixed(4)} = <span class="rs">${pAgB.toFixed(4)}</span>  [${(pAgB * 100).toFixed(1)}%]

      <span class="cm">// Condicional inversa — dado que A ocurrió, ¿qué tan probable es B?</span>
      P(B|A) = P(A∩B)/P(A) = ${pAB.toFixed(4)}/${pA.toFixed(4)} = <span class="rs">${pBgA.toFixed(4)}</span>  [${(pBgA * 100).toFixed(1)}%]
    </div>
  `;
}
...
function updateIC() {
  ...
  // IC Proporción
  const ph = s.pBien,
    se_p = Math.sqrt(ph * (1 - ph) / n),
    me_p = z * se_p;
  const lo_p = Math.max(0, ph - me_p),
    hi_p = Math.min(1, ph + me_p);
...
Interpretación: Con 95% de confianza, entre <span class="rs">${(lo_p * 100).toFixed(1)}%</span> y <span class="rs">${(hi_p * 100).toFixed(1)}%</span> de los estudiantes duerme bien.</div>`;
  drawBellChart('chart-ic-prop', ph, se_p, lo_p, hi_p, 'p̂=' + ph.toFixed(3), { fill: 'rgba(96,165,250,.25)', out: 'rgba(96,165,250,.04)', line: 'rgba(96,165,250,.9)' });

  // IC Media
  const xbar = s.mean,
    se_m = s.std / Math.sqrt(n),
    me_m = z * se_m,
    lo_m = xbar - me_m,
    hi_m = xbar + me_m;
...
Interpretación: Con 95% de confianza, el promedio real de horas de sueño está entre <span class="rs">${lo_m.toFixed(2)}</span> y <span class="rs">${hi_m.toFixed(2)}</span> horas.</div>`;
  drawBellChart('chart-ic-media', xbar, se_m, lo_m, hi_m, 'x̄=' + xbar.toFixed(2) + 'h', { fill: 'rgba(74,222,128,.25)', out: 'rgba(74,222,128,.04)', line: 'rgba(74,222,128,.9)' });
...
function loadHypFromData() {
  const s = stats();
  if (!s.n) { notify('Primero carga datos en la pestaña Datos'); return; }
...
function updateResumen() {
  ...
  const z = +document.getElementById('conf-level').value || 1.96;
  const ph = s.pBien,
    se_p = Math.sqrt(ph * (1 - ph) / s.n),
    me_p = z * se_p;
  const lo_p = Math.max(0, ph - me_p),
    hi_p = Math.min(1, ph + me_p);
  const se_m = s.std / Math.sqrt(s.n),
    me_m = z * se_m;
  const zStat = (ph - 0.5) / Math.sqrt(0.25 / s.n),
    reject = Math.abs(zStat) > 1.96;
...
function tokenize(text) {
  ...
  while (i < text.length) {
    ...
    if (nm && (i === 0 || !/[A-Za-z(₀₁₂]/.test(text[i - 1]))) { tokens.push({ type: 'num', val: nm[0] });
      i += nm[0].length; continue; }
    const pm = text.slice(i).match(/^P\([^)]+\)/);
    if (pm) { tokens.push({ type: 'label', val: pm[0] });
...
function drawOnBoard(boardId, idleId, sdotId, stxtId, sstepId, formulaNombre, formulaGeneral, result, videoTopic) {
  const bid = boardId;
  ...
  title.textContent = formulaNombre + ' — paso a paso';
  ...
  bc.appendChild(fDiv);
  setTimeout(() => fDiv.classList.add('show'), 300);

  // Sustitución animada
  const sDiv = document.createElement('div');
  sDiv.className = 'formula-display';
  ...
  stepsDiv.className = 'steps-area';
  bc.appendChild(stepsDiv);
  ...
  setTimeout(() => {
    fDiv.style.transition = 'opacity .4s';
    fDiv.style.opacity = '.25';
    sDiv.innerHTML = '';
    sDiv.style.opacity = '1';
    sDiv.style.transition = 'opacity .5s';
    ...
    tokens.forEach((tok, i) => {
      ...
      if (tok.type === 'num') {
        ...
        setTimeout(() => { sp.style.transition = 'opacity .3s,transform .4s cubic-bezier(.34,1.56,.64,1)';
          sp.style.opacity = '1';
          sp.classList.add('tok-anim'); }, i * 65 + 200);
      } else if (tok.type === 'label') {
        sp.style.cssText = 'color:var(--chalk-c);font-weight:700;font-family:Kalam,cursive;font-size:32px';
        sp.textContent = tok.val;
      } else {
        sp.style.cssText = 'color:var(--chalk-dim);font-family:Kalam,cursive;font-size:32px';
        sp.textContent = tok.val;
      }
      sDiv.appendChild(sp);
    });
  ...
  setTimeout(() => {
    ...
    // Agregar botón "Ver video"
    const videoBtn = document.createElement('button');
    videoBtn.className = 'hero-btn-ghost';
    videoBtn.style.cssText = 'margin-top: 16px; padding: 8px 16px; font-size: 12px; align-self: flex-start; border-color: rgba(255,255,255,.15); color: var(--chalk-dim);';
    videoBtn.innerHTML = `
    ...
    const videoUrl = getVideoUrl(videoTopic);
    const videoTitle = getVideoTitle(videoTopic);
    
    if (videoUrl) {
      videoBtn.onclick = () => openVideoModal(videoUrl, videoTitle);
      videoBtn.style.opacity = '0';
      videoBtn.style.transform = 'translateY(10px)';
      setTimeout(() => {
        videoBtn.style.transition = 'all 0.4s ease';
      ...
      videoBtn.title = 'Video no disponible';
    }
    
    const bcContainer = document.getElementById(boardId);
    bcContainer.appendChild(videoBtn);
...
function clearBoardById(boardId) {
  ...
  Array.from(bc.children).forEach(c => { if (!c.classList.contains('board-idle-state')) c.remove(); });
  const idle = bc.querySelector('.board-idle-state');
  if (idle) idle.style.opacity = '1';
  const prefix = boardId.replace('board-', '');
  const sdot = document.getElementById('sdot-' + prefix);
  if (sdot) sdot.classList.remove('active');
  const stxt = document.getElementById('stxt-' + prefix);
  if (stxt) stxt.textContent = 'Listo';
  const sstep = document.getElementById('sstep-' + prefix);
  if (sstep) sstep.textContent = '';
}
...
const DESC_FORMULAS = {
  ...
  mediana: {
    nombre: 'Mediana',
    general: 'Me = valor central de datos ordenados',
  ...
  std: {
    nombre: 'Desviación estándar (s)',
    general: 's = √[ Σ(xᵢ − x̄)² / (n−1) ]',
    ...
    solve: (v) => { const var_ = v.sv / (v.n - 1),
        std = Math.sqrt(var_); return { steps: [
...
function loadDescFromData() {
  ...
  let e, e2;
  if (f === 'media') {
    e = document.getElementById('df-suma');
    if (e) e.value = suma.toFixed(2);
    e2 = document.getElementById('df-n');
    if (e2) e2.value = s.n;
  } else if (f === 'mediana') {
    const sorted = [...horas].sort((a, b) => a - b);
    const n = sorted.length;
    e = document.getElementById('df-pos1');
    if (e) e.value = n % 2 === 0 ? sorted[n / 2 - 1] : sorted[Math.floor(n / 2)];
    e2 = document.getElementById('df-pos2');
    if (e2) e2.value = n % 2 === 0 ? sorted[n / 2] : 0;
  } else if (f === 'std' || f === 'varianza') {
    e = document.getElementById('df-sv');
    if (e) e.value = sv.toFixed(4);
    e2 = document.getElementById('df-n');
    if (e2) e2.value = s.n;
  }
...
function solveBoardDesc() {
  ...
  drawOnBoard('board-desc', 'idle-desc', 'sdot-desc', 'stxt-desc', 'sstep-desc', f.nombre, f.general, result, currentDescFormula);
}
...
const PROB_FORMULAS = {
  laplace: {
    ...
    general: 'P(A) = casos favorables / casos posibles',
  ...
  union_excl: {
    nombre: 'Unión — eventos excluyentes',
    general: 'P(A∪B) = P(A) + P(B)',
  ...
  union_noexcl: {
    nombre: 'Unión — eventos no excluyentes',
    ...
    solve: (v) => { const s2 = v.pA + v.pB,
        res = s2 - v.pAB; return { steps: [
  ...
  conjunta_indep: {
    nombre: 'Conjunta — independientes',
    general: 'P(A∩B) = P(A) · P(B)',
  ...
  diferencia: {
    nombre: 'Diferencia de eventos',
    general: 'P(A−B) = P(A) − P(A∩B)',
  ...
  prob_total: {
    ...
    general: 'P(B) = P(B|A₁)·P(A₁) + P(B|A₂)·P(A₂)',
    ...
    solve: (v) => { const t1 = v.pBdA1 * v.pA1,
        t2 = v.pBdA2 * v.pA2,
        res = t1 + t2; return { steps: [
  ...
  bayes: {
    ...
    solve: (v) => { const num = v.pBdA1 * v.pA1,
        den = v.pBdA1 * v.pA1 + v.pBdA2 * v.pA2,
        res = num / den; return { steps: [
...
function renderProbInputs() {
  ...
  document.getElementById('prob-board-fields').innerHTML = f.fields.map(fi => `
    <div class="bi-field">
      <label><span class="bi-badge ${fi.color === 'b' ? 'b' : fi.color === 'r' ? 'r' : ''}">${fi.color === 'a' ? 'A' : fi.color === 'b' ? 'B' : '∩'}</span> ${fi.label}</label>
      <input type="number" id="pf-${fi.key}" step="0.001" min="${fi.key === 'fav' || fi.key === 'pos' ? 0 : 0}" max="${fi.key === 'fav' || fi.key === 'pos' ? 99999 : 1}" placeholder="${fi.ph}" value="" ${hasData ? '' : 'disabled'} style="opacity: ${hasData ? 1 : 0.5};">
    </div>`).join('');

  const exHTML = hasData ? f.ejemplos.map(ej => `<span class="bi-ex" onclick='loadProbEx(${JSON.stringify(ej.vals)})'>${ej.label}</span>`).join('') :
    '<span style="color: var(--muted); font-size: 11px;">⚠️ Carga datos en la pestaña "Datos" para usar ejemplos reales</span>';
  document.getElementById('prob-board-ex').innerHTML = exHTML;
}

function loadProbEx(vals) {
...
function solveProb() {
  ...
  const vals = {};
  for (const fi of f.fields) {
    const el = document.getElementById('pf-' + fi.key);
    if (!el || el.value === '') { notify('Completa todos los campos'); return; }
    vals[fi.key] = parseFloat(el.value);
  }
  const result = f.solve(vals);
  drawOnBoard('board-prob', 'idle-prob', 'sdot-prob', 'stxt-prob', 'sstep-prob', f.nombre, f.general, result, currentProbFormula);
}
...
const IC_FORMULAS = {
  ic_prop: {
    nombre: 'IC para proporción',
    general: 'IC₉₅% = p̂ ± z · √(p̂(1−p̂)/n)',
    ...
    solve: (v) => { const se = Math.sqrt(v.ph * (1 - v.ph) / v.n),
        me = v.z * se,
        lo = Math.max(0, v.ph - me),
        hi = Math.min(1, v.ph + me); return { steps: [
  ...
  ic_media: {
    ...
    ejemplos: [{ label: 'De mis datos', auto: true }],
    solve: (v) => { const se = v.s / Math.sqrt(v.n),
        me = v.z * se,
        lo = v.xbar - me,
        hi = v.xbar + me; return { steps: [
...
function loadICFromData() {
  const s = stats();
  if (!s.n) { notify('Primero carga datos'); return; }
  let e, e2, e3, e4;
  if (currentICFormula === 'ic_prop') {
    e = document.getElementById('icf-ph');
    if (e) e.value = s.pBien.toFixed(4);
    e2 = document.getElementById('icf-n');
    if (e2) e2.value = s.n;
    e3 = document.getElementById('icf-z');
    if (e3) e3.value = 1.96;
  } else {
    e = document.getElementById('icf-xbar');
    if (e) e.value = s.mean.toFixed(4);
    e2 = document.getElementById('icf-s');
    if (e2) e2.value = s.std.toFixed(4);
    e3 = document.getElementById('icf-n');
    if (e3) e3.value = s.n;
    e4 = document.getElementById('icf-z');
    if (e4) e4.value = 1.96;
  }
  notify('Datos cargados de la encuesta');
}
...
function solveIC_board() {
  ...
  const vals = {};
  for (const fi of f.fields) {
    const el = document.getElementById('icf-' + fi.key);
    if (!el || el.value === '') { notify('Completa todos los campos'); return; }
    vals[fi.key] = parseFloat(el.value);
  }
  const result = f.solve(vals);
  drawOnBoard('board-ic', 'idle-ic', 'sdot-ic', 'stxt-ic', 'sstep-ic', f.nombre, f.general, result, currentICFormula);
}
...
function solveHyp() {
  ...
  drawOnBoard('board-hyp', 'idle-hyp', 'sdot-hyp', 'stxt-hyp', 'sstep-hyp', 'Prueba Z para proporción', 'z = (p̂ − p₀) / √(p₀(1−p₀)/n)', result, 'test_z');
}
...
function notify(msg) {
  ...
  clearTimeout(notifT);
  notifT = setTimeout(() => el.classList.remove('show'), 2200);
}
